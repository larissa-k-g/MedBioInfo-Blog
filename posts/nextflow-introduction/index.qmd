...
4. Now, the first process block is defined. In this example it is called "Splitletters" and is defined inbetween the  brackets {}. As an input a value is expected which has the variable X assigned to it. However, this could also be a file or path or the like. The output is defined right after. Here, it's a path. All outputs from this process should be called 'chunk_*', where * is a simple placeholder. Next, the script defines a two parted process of printing the value X and then split it into groups of 6 characters and to save it one by one in a file called chunk_aa.  
```script   
